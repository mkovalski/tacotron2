...
for word in paths:
    ...
    with open('sc_audio_text_train_filelist.txt', 'a') as myFile:
        for idx in indices[0:train_stop]:
            myFile.write(os.path.join(curr_dir, files[idx]) + '|' + word + '\n')

    with open('sc_audio_text_val_filelist.txt', 'a') as myFile:
        for idx in indices[train_stop:val_stop]:
            myFile.write(os.path.join(curr_dir, files[idx]) + '|' + word + '\n')
    
    with open('sc_audio_text_test_filelist.txt', 'a') as myFile:
